...
void readargs(struct config *c, int ac, char **av) {
  ...
  while ((ch = getopt(ac, av, "Hf:s:c:h:i:p:t:m:e:z:w:")) != -1) {
    switch (ch) {
      case 'H':
	c->printheader = 1;
	break;
...
//Timing infor:
// It takes 340876118 to reach the line to monitor the attack

int main(int ac, char **av) {
  /*
  uint64_t current_time;
  current_time = rdtscp64();
  printf("fr time 1 : ");
  printf("%"PRIu64"\n", current_time);
  */
 ...
 //char f_name[100];
 //sprintf(f_name, "./Traces/%s/tr%d.txt",c.measname,c.measidx);
 //FILE * fp;
 //fp = fopen(f_name, "w");
  for (int i = 0; i < l; i++) {
    for (int j = 0; j < c.nmonitored; j++){
      printf("%d ", res[i * c.nmonitored + j]);
      //fprintf(fp,"%d ", res[i * c.nmonitored + j]);
    }
    //fprintf(fp, "\n" );
   putchar('\n');
  }
